...
기본 전략 (수정하려면 아래 상수만 변경):
  ...
  - 정체 보호: 트레일링 매도가 한 번 나간 뒤 그보다 더 높은 새 고점 없이 가격이 계속 흘러내리면
    ...
    쓰인 트리거선(고점-2%p 또는 보호선)보다 추가로 -6%p(STALL_GAP) 더 밀리면 잔여 전량 청산.
...
#  - ANOMALY_RATE : -3% 손절이 30초마다 도는 구조상 이만큼 깊은 손실률은 정상 경로로 도달 불가.
