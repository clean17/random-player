...
CASH_DEPLOY_RATIO = 0.75 if _IS_MOCK_ENV else 0.65   # 가용 현금 중 자동매수에 쓸 최대 비율.
                           ...
                           #    꺼져 있음) — 설정 조합이 다른데 값만 그대로 갖다 썼더니 목표(현금
                           #    20%)와 실측(현금 약 31%)이 어긋났다. mock 전용 값을 다시 검증했다.
                           #
                           # [real] 0.65 유지 — 아래는 real 설정(슬롯5/divisor5/초기자본 199만원)
                           #   기준 부트스트랩 25회(하루 후보 80% 표집, 후보 상위 7개, 왕복비용 0.2%):
                           ...
                           # real fire 매수를 되살릴 때 이 값을 그대로 쓰면 된다(재검증까지는).
                           #
                           # [mock] 0.75로 변경(0.65→0.75, 사용자 요청 '현금 25% 정도로') — mock
                           #   설정(슬롯20/divisor20/초기자본 2,400만원, 후보 상위 20개)으로 다시
                           #   부트스트랩 30회(하루 후보 80% 표집):
                           #   ratio 0.65(구) → 현금 30.0%  평균 -1.2%  최저  -9.8%  음수 53%  MDD -32.8%
                           #   ratio 0.70     → 현금 27.3%  평균 -1.7%  최저 -10.8%  음수 57%  MDD -33.9%
                           #   ratio 0.75(신) → 현금 24.9%  평균 -2.6%  최저 -12.0%  음수 63%  MDD -34.8%  ← 채택
                           # ⚠️ 이 구간(229거래일) 데이터에서는 슬롯20 설정 자체가 전부 마이너스로
                           #    나오고, 노출을 늘릴수록(현금을 줄일수록) 일관되게 더 나빠진다 —
                           #    real과 같은 방향('노출을 늘리는 쪽이 항상 불리하다'). 25% 현금은
                           #    성과 최적점이 아니라 요청받은 목표치이고, 그 대가가 위 표다.
